...
void test_CurrentLimiter_selectMinCurrent(void)
{
    ...
    can_bus.bms_current_limit.max_discharge_current = f29bms_dbc_current_limit_max_discharge_current_encode(MIN_ALLOWED_CURRENT_LIMIT+10);
    ...
    char err_msg[150];
    sprintf(err_msg, "Current limit set above maximum allowed current limit.(%fA)", f29bms_dbc_current_limit_max_discharge_current_decode(can_bus.bms_current_limit.max_discharge_current));
    TEST_ASSERT_MESSAGE(f29bms_dbc_current_limit_max_discharge_current_decode(can_bus.bms_current_limit.max_discharge_current) == MAX_ALLOWED_CURRENT_LIMIT, err_msg);
    ...
    CurrentLimiter_10Hz(&bm);
    TEST_ASSERT_MESSAGE(f29bms_dbc_current_limit_max_discharge_current_decode(can_bus.bms_current_limit.max_discharge_current) == MIN_ALLOWED_CURRENT_LIMIT + 50, "Current limit not set to lower limit value (calculated).");
    ...
    CurrentLimiter_10Hz(&bm);
    TEST_ASSERT_MESSAGE(f29bms_dbc_current_limit_max_discharge_current_decode(can_bus.bms_current_limit.max_discharge_current) == MIN_ALLOWED_CURRENT_LIMIT + 25, "Current limit not set to lower limit value (correction).");
    ...
    CurrentSense_get_current_ExpectAnyArgsAndReturn(true);
    CurrentLimiter_10Hz(&bm);
    TEST_ASSERT_MESSAGE(f29bms_dbc_current_limit_max_discharge_current_decode(can_bus.bms_current_limit.max_discharge_current) == MIN_ALLOWED_CURRENT_LIMIT, "Current limit set below minimum allowed current limit.");
}