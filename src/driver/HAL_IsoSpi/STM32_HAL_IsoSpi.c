...
#define ISOSPI_PINS 0xF00

void HAL_IsoSpi_Init()
{
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_SPI2, ENABLE); //Using SPI 2
    RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE);
    GPIO_PinAFConfig(GPIOB, ISOSPI_PINS, GPIO_Mode_AF);
    GPIO_InitTypeDef isospiGPIOinit = {ISOSPI_PINS, GPIO_Mode_AF, GPIO_Speed_Level_3, GPIO_OType_PP, GPIO_PuPd_NOPULL}; //Not sure about speed, OType, and PuPd
    GPIO_Init(GPIOB, &isospiGPIOinit);
