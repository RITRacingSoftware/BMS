...
def test_current_limiting(sim):
    ...
    # Set up cell voltage and current to values that shouldn't result in current limiting
    for i in range(0, 90):
        sim.stage_cell_info(i, 4.0, True)
    ...
    for i in range(0,1000):
        sim.tick()
        current_limit = sim.signals['MaxDischargeCurrent']
        if current_limit != max_current_limit:
            break
    assert 'MaxDischargeCurrent' in sim.signals, "MaxDischargeCurrent never sent"
